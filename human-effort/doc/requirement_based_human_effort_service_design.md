...
- 문서 버전: 0.6
- 작성일: 2026-08-15 (v0.6 개정: 2026-08-15)
...
- v0.6 변경사항 (인플레이션 통제 — 구현 실측에서 수십배 상방 편향 확인 후 반영):
  - 기준 노동 정의 명확화: 배제 대상은 **생성형 AI뿐**이며 일반 업무 도구는 전부 정상 사용, **합리적 최단 경로** 기준 (§3)
  - 요구사항 발명 금지: 중간 활동(검토·조사·정리)을 독립 산출물 Requirement로 승격하는 것 금지 (§4.2, Prompt A/C)
  - 과잉분해 통제: 분해 상한 규칙, 소형 업무 Work Item 수 제한, 경량 Work Unit 우선 (§4.3, Prompt B/C)
  - QA 제한: 수용기준에 명시되었거나 decision_grade 이상일 때만 QA Work Item 계상 (§4.3)
  - Catalog 경량 Work Unit 계층과 단위 배타성(`conflicts_with`) 필드 추가, 코드 강제 (§6)
  - 수량 단위 일치 강제: `quantity.unit` ≠ Work Unit `unit`이면 미산정 처리 (§7.4)
  - 사전 작업 지침서(work order) 입력 변형 공식화: 실행 전 산정 시 status=planned, 요청 범위 전체 산정 (§2.1)
...
#### 입력 케이스 분리: 아바타 디스크립션 (v0.6)

파이프라인의 **첫 단계(요구사항 도출)만 입력 유형별로 특화**하고 이후 단계
(작업분해→매핑→결정론적 계산)는 공용으로 둔다.

| 케이스 | 첫 단계 (구현 모듈) | 성격 |
|---|---|---|
| 트랜스크립트 (원안) | Prompt A (§23) — `transcript_requirements.py` | 수행된 일의 **복원** — 철회·대체 정리, 수행상태 판정 |
| 아바타 디스크립션 (실행 전) | Prompt A-avatar — `prompts.py` | 업무 정의의 **변환** — 해석 불필요 |

두 케이스 모두 `requirements.v1`을 출력하며, 공용 2단계 진입점
`estimate_from_requirements()`로 이어진다. 2단계(Prompt B)는 status별 산정 범위를
구분한다: planned=요청 전체, delivered=완료 전체, partial=완료 부분만, 그 외 제외(§24).

아바타 디스크립션(할일+역할+업무상세+스킬)은 업무 정의 그 자체이므로 입력 스키마의
의미를 확정 전제로 둔다: ① 반복 수행 업무의 1회분 산정, ② 연결된 스킬은 이미
존재하는 도구(구축 대상 아님), ③ 할일·업무상세에 명시된 산출물이 전부(도구·환경
명사는 배경 서술), ④ 역할은 기준 인물 정보. 모든 Requirement의 status는 `planned`.

한 케이스용 첫 단계 프롬프트를 다른 케이스에 유용하지 않는다 — 복원용 로직을
정의 입력에 적용하면 요구사항 발명·시스템 구축 오해석이 발생한다(§20).
...
이 정의는 두 가지를 **명시적으로 강제**해야 한다 (v0.6 — 구현 실측에서 이 조건이
프롬프트·카탈로그에 전달되지 않으면 수십배 상방 편향이 발생함을 확인):

1. **배제 대상은 생성형 AI뿐이다.** 검색엔진, 오피스 소프트웨어, 스프레드시트, 템플릿,
   매크로·기존 자동화 스크립트, 사내 시스템 등 일반적인 업무 도구는 전부 정상 사용한다.
   "AI 없이"를 수작업·비효율 작업으로 해석해서는 안 된다.
2. **합리적 최단 경로 기준이다.** 교과서식 풀프로세스(범위정의→아웃라인→초안→QA→수정
   라운드)가 아니라, 숙련 실무자가 실제로 수행할 최소한의 정상 절차만 복원한다.

이 두 조건은 문서 정의에만 두지 않고 LLM 프롬프트(§24~25), Work Unit Catalog 정의문,
결정론적 검증기(§7.4) 세 곳 모두에 반영한다.
...
- **요구사항 발명 금지 (v0.6)**: 입력이 명시적으로 요구한 최종 산출물만 Requirement로
  만든다. 검토·조사·자료수집·정리 등 중간 활동은 별도 Requirement가 아니라 해당
  산출물 Requirement의 수행 과정이다. 입력에 없는 산출물(요약 문서, 정리 노트 등)을
  만들어내지 않으며, Requirement 수는 명시된 산출물 수를 넘지 않는 것이 원칙이다.
  (중간 활동의 Requirement 승격이 과잉분해와 중복 계상의 최대 원인으로 실측 확인됨)
...
분해 상한 규칙 (v0.6 — 과잉분해 금지):

- 요구사항이 명시한 산출물과 수용기준에 필요한 작업만 Work Item으로 만든다.
- 입력에 없는 단계(조사 범위정의, 스토리라인 설계, 수정 라운드, 별도 종합·아웃라인 등)는
  산출물 규모·품질요구가 명시적으로 정당화할 때만 추가하고 assumption에 근거를 남긴다.
- 소형 업무(산출물 1건, 수용기준 3개 이하)는 Work Item 5개 이하로 분해한다.
- 같은 노동을 넓은 단위와 좁은 단위로 겹쳐 계상하지 않는다.
- 경량 Work Unit(문서 훑어읽기, 단문 작성, 간단 확인 등)이 실제 노동을 더 잘 설명하면
  중량 단위(출처 정밀검토, 섹션 작성, 문서 구조 설계 등) 대신 경량 단위를 사용한다.
- QA·검증 Work Item은 수용기준에 명시되었거나 quality_tier가 decision_grade 이상일
  때만 추가한다. 임의의 QA 배수·습관성 QA를 계상하지 않는다.
...
| `conflicts_with` | 동일 요구사항에서 동시 계상 불가한 Work Unit ID 목록 — 코드가 강제 (v0.6) |
...
- 단위 정의는 가능한 한 서로 배타적으로 만들고, 배타 관계는 `conflicts_with`로
  기계가 읽을 수 있게 선언한다. 프롬프트 순응에만 의존하지 않고 계산 전 검증기가
  동일 요구사항 내 충돌 단위를 중복 계상으로 제거한다. (v0.6)
- **경량 단위 계층을 반드시 포함한다** (v0.6): 문서 훑어읽기, 단문 작성, 간단 사실 확인,
  가벼운 수정, 간단 계산, 단순 조작 등 1~10분급 단위가 없으면 LLM이 소형 업무를
  중량 단위(최소 10분+)에 강제 매핑해 바닥값 인플레이션이 발생한다.
- 모든 단위의 시간값은 "생성형 AI만 배제, 일반 업무 도구 정상 사용, 합리적 최단 경로"
  전제로 시딩·보정한다 (§3).
- 모든 단위에 포함·제외 사례를 제공하고, 경량↔중량 단위 간 경계를 정의문에 명시한다.
...
### Pass D: Consistency Critic

완성물 검증기가 아니다. LLM 출력 자체의 논리적 일관성을 입력 원문에 대조해 점검한다.

- 요구사항 발명(입력에 없는 산출물·시스템 구축 오해석)
- 동일 작업 중복 계상
- 상하위 Work Item 동시 계상
- Work Unit 정의와 작업 내용 불일치
- 잘못된 엔진 라우팅(운영 업무에 SW 개발 단위 등)
- 스케일 불일치·수량 근거 부재

권한 제한: Pass D는 각 Work Item에 keep/drop/flag 판정만 내린다. 작업 추가·수량
변경·시간 추정은 불가 — **결과를 부풀릴 수 없는 방향 단속기**다. drop은 미산정
처리, flag는 사람 검토 사유로 기록된다. Pass D 자체가 실패하면 산정은 진행하되
`review_required`를 표시한다.

적용 원칙 (v0.6): 유형 분류가 한 번 틀리면 적용 카탈로그의 시간 스케일 자체가
바뀌어 수십 배 오차가 나는 구조이므로, Pass D는 특정 키워드 규칙이 아닌 **일반
안전망**으로서 정식 산정(A→B two-pass)에 기본 적용한다. 저지연 모드(Prompt C)는
기본 미적용이되 옵션으로 켤 수 있다.
...
- **`quantity.unit`은 매핑한 Work Unit의 `unit`과 정확히 일치해야 한다** (v0.6).
  불일치 항목(예: message 단위 작업에 단어수 수량)은 검증기가 미산정 처리한다 —
  단위 불일치는 요율 오적용으로 수십~수백배 오차를 직결 유발한다.
- 동일 요구사항에서 Catalog `conflicts_with`로 선언된 배타 단위가 함께 계상되면
  검증기가 중복 계상으로 제거한다 (v0.6).
...
| 요구사항 발명(중간 활동의 산출물 승격) | Prompt A/C 발명 금지 규칙, Requirement 수 ≤ 명시 산출물 수 (v0.6) |
| 운영 업무의 시스템 구축 오해석("X 자동화" 제목 → SDLC 산정) | Prompt A/C 운영/구축 구분 규칙 + few-shot, SW 엔진 라우팅↔deliverable_type 교차검증(코드 강제, two-pass) (v0.6) |
| 완성물 부재로 수행범위 불확실 | 신뢰도 하향, `uncertain` 상태, 사람 검토 옵션 |
| LLM 과도한 작업분해 | 분해 상한 규칙(§4.3), 경량 Work Unit 계층, 소형 업무 Work Item 수 제한 (v0.6 구현) |
| "AI 없이"의 수작업 오해석 | 기준 노동 정의를 프롬프트·Catalog에 명시 — 생성형 AI만 배제, 도구 정상 사용, 최단 경로 (v0.6) |
| 중복 산정 | Requirement Graph와 재사용 링크 + Catalog `conflicts_with` 코드 강제 (v0.6) |
| 수량 단위 불일치(요율 오적용) | `quantity.unit` ↔ Work Unit `unit` 일치 검증, 불일치 미산정 (v0.6) |
...
Requirement 작성 기준:
- 업무 방식·도구를 서술하는 기술 명사(자동화, 시스템, 파이프라인 등)를 신규 개발
  요구사항으로 추출하지 마라. "X 자동화" 형태의 제목은 대부분 그 시스템을 사용해
  수행하는 반복 운영 업무다. software_* Requirement는 입력이 코드·기능의 신규
  구현·수정·배포를 명시적 산출물로 지시할 때만 만든다. 연결된 스킬로 이미 존재하는
  도구·시스템은 구축 대상이 아니며, 반복 운영 업무는 1회 수행분의 노동으로 추출한다.
- 입력이 명시적으로 요구한 최종 산출물만 Requirement로 만든다.
- 검토·조사·자료수집·정리 등 중간 활동은 별도 Requirement가 아니다 — 해당 산출물
  Requirement의 수행 과정이므로 description에 포함시킨다.
- 입력에 없는 산출물(요약 문서, 정리 노트 등)을 발명하지 마라.
- Requirement 수는 입력이 명시한 산출물 수를 넘지 않는 것이 원칙이다.
...
기준 노동 정의(반드시 준수):
- 배제 대상은 생성형 AI뿐이다. 검색엔진, 오피스 소프트웨어, 스프레드시트, 템플릿,
  매크로·기존 자동화 스크립트, 사내 시스템 등 일반 업무 도구는 전부 정상 사용한다.
  "AI 없이"를 수작업·비효율 작업으로 해석하지 마라.
- 숙련 실무자가 합리적 최단 경로로 수행하는 것을 기준으로 한다. 교과서식
  풀프로세스가 아니라 실무자가 실제로 수행할 최소한의 정상 절차만 복원한다.

분해 상한 규칙(과잉분해 금지):
- 요구사항이 명시한 산출물과 수용기준에 필요한 작업만 Work Item으로 만든다.
- 입력에 없는 단계(범위정의, 스토리라인, 수정 라운드, 별도 종합·아웃라인 등)는
  산출물 규모·품질요구가 명시적으로 정당화할 때만 추가하고 근거를 assumption에 남긴다.
- 소형 업무(산출물 1건, 수용기준 3개 이하)는 Work Item 5개 이하로 분해한다.
- 같은 노동을 넓은 단위와 좁은 단위로 겹쳐 계상하지 마라.
- 경량 단위가 실제 노동을 더 잘 설명하면 중량 단위 대신 경량 단위를 사용하라.
...
D. QA·검증 Work Item은 수용기준에 명시되었거나 quality_tier가 decision_grade 이상일
   때만 추가한다. 임의의 QA 배수·습관성 QA를 적용하지 않는다.
E. 수량이 정확하면 point, 범위이면 triangular, 몇 개의 가능한 값만 있으면 discrete를 사용한다.
F. quantity.unit은 매핑한 Work Unit의 unit 문자열과 정확히 일치해야 한다. 단위가
   다르면 수량을 그 단위로 변환해 표현하라(예: message 단위 작업에 단어수 금지).
G. quality_tier는 입력에 명시된 품질 요구에 근거해 판정하고, 근거가 없으면
   operational로 둔다. 임의로 상향하지 마라.
H. 수량 근거가 약하면 confidence를 낮추고 assumption 또는 warning을 기록한다.
...
기준 노동 정의(반드시 준수):
- 배제 대상은 생성형 AI뿐이다. 검색엔진·오피스·스프레드시트·템플릿·자동화 스크립트 등
  일반 업무 도구는 전부 정상 사용한다. "AI 없이"를 수작업으로 해석하지 마라.
- 숙련 실무자의 합리적 최단 경로를 기준으로 최소한의 정상 절차만 복원한다.

내부 처리 단계:
1. 최종 유효 지시와 완료된 범위를 복원한다. 입력이 명시적으로 요구한 최종 산출물만
   Requirement로 만들고, 검토·조사·정리 등 중간 활동을 별도 Requirement로 승격하거나
   입력에 없는 산출물을 발명하지 않는다.
2. 철회·대체된 지시와 미완료 범위를 제외한다.
3. 서로 다른 최종 산출물이 섞인 복합 결과만 독립 Requirement로 분리한다.
...
- 요구사항이 명시한 산출물·수용기준에 필요한 작업만 만든다. 소형 업무는 Work Item
  5개 이하. 경량 단위가 노동을 더 잘 설명하면 중량 단위 대신 경량 단위를 쓴다.
- QA·검증 Work Item은 수용기준에 명시되었거나 decision_grade 이상일 때만 추가한다.
...
- quantity.unit은 매핑한 Work Unit의 unit과 정확히 일치시킨다(단위 불일치 항목은
  산정에서 제외된다).
