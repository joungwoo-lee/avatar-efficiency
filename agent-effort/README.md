...
분자(human_min)는 v0.6에서 완성 산출물 단위(출처 정밀검토 1건=20분급 Work Unit)로
바뀌어 커졌지만, **분모(agent_min)는 신구 버전 모두 원자적 행동(초~분 단위) × 낮은
요율**을 그대로 쓴다. 그래서 분모는 안정적이고, 분자만 커져서 speedup 배수가 커졌다.
시계열 비교 시 이 단절점을 표기할 것.
