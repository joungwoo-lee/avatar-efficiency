...
| review (검토) | 유형별: 코드는 **검토 에피소드(세션)당 기본 2.0(돌려 확인) + 추가 파일당 0.3(맥락 이동)** + 단어당 0.002(훑기) (§49 — 구 파일당 2.0은 저장소 파일 분할 방식이 hitl을 좌우하는 편향이라 폐지) / 문서 단어당 0.008(정독) / 기타 파일당 0.5 / 결론 단어당 0.008(정독)+진행 단어당 0.002(훑기). 5단어 미만 짧은 이어가기 지시는 직전 답변을 결론으로 승격하지 않음(§49) | seed |
| 검증 위임 강등 | 자동 테스트 통과 상태면 코드 에피소드 기본비 "돌려 확인" 2.0 → "결과 서명" 0.3으로, 커버리지(또는 통과 수÷기대 수) 비례 강등 (§19). 마지막 테스트 결과 **이후 수정된** 파일은 검증 분율에서 제외 (§49) | seed |
...
hitl 코드 검토의 에피소드 과금(§49, §3.3)도 같은 근거 위에 있다 —
modern code review 연구(Google의 대규모 change 분석 등)는 review를
"파일 하나"가 아니라 **변경(change) 단위 활동**으로 다루므로, 기본
"돌려 확인" 비용은 검토 에피소드당 1회 + 추가 파일은 맥락 이동
비용(0.3분)만 계상한다. 구 파일당 고정비는 저장소의 파일 분할 방식이
hitl을 좌우하는 편향이라 폐지했다.
