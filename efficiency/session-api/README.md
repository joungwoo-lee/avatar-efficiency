...
| `record_actions_code_api.py` | **LLM 0회 API** (§32) — 분자까지 코드 실측(읽기 항해 구조 + 쓰기 순계 + 건수형은 행동 순계(§46~48): 검색=착지-기여 문서당 1건·실행=명령 신원당 1건(실패 호출 상쇄), 하한 1. 셸 속 grep·sed류는 검색·읽기 축으로 재분류(§47)). **기능·휴먼화 원리·요율 통합 문서: [record_actions_code_api.md](record_actions_code_api.md)**. `humanize=False`(`--raw`)로 휴먼화 기능 끈 대조군 실행 가능 |
...
- **산출물 장황함은 효율을 부풀린다** (과정 장황함과 구분할 것):
  - AI가 장황하게 **돌아다니는** 것(헛읽기·재검색·재실행·번복)은 휴먼화가
    분자에서 걷어내고 비용은 분모에 남아 → 효율 하락. 정상 벌점.
  - 그러나 **결말에 살아남은 산출물**이 필요 이상으로 크면 — 사람이 코드
    한 줄로 해결할 것을 에이전트가 모듈로 구현, 200단어면 될 보고를
    2,000단어로 — 분자가 "사람이 그 분량을 만드는 시간"(비싼 요율)으로
    커지고 분모는 싼 요율(AI 출력+검토)로만 커져 → 효율 상승. 측정기는
    "그 분량이 필요했는가"를 판정하지 않는다 (req-actions의 §30 명시 수량
    닻 같은 요구 분량 상한이 record 계열에는 없음). 산출물이 유난히 큰
    세션의 효율은 이 왜곡을 의심할 것.
