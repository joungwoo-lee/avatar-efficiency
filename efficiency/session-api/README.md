...
"숫자 결정권을 코드로 옮긴 곳만 결정론이 된다"의 실측 증명.

**순서의 성격 (§41 정정)**: rec도 "사람이 했다면"의 시뮬이므로
**req ≤ rec는 논리 보장이 아니라 설계 비대칭(입력 앵커링·상한 유무)이
만드는 경향**이다. 논리적으로 보장되는 것은 결정론 샌드위치 —
`code(직행 바닥, 코드) ≤ LLM 자들(req·rec) ≤ 로레코드(궤적 천장, 코드)`.
판독 기준: **대폭·지속적인 역전**(예: 수 배 차이가 세션 유형 단위로
반복)은 측정 버그 신호 (§29·§30·§33 사고가 전부 이렇게 발견됨),
**근소한 일회성 역전**(±수 분)은 LLM 표본 노이즈로 정상.
