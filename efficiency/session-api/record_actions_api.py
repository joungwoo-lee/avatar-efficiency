...
기준선.** 프롬프트는 직행을 지시하지만, 입력 요약의 활동 신호(검색·실행·
왕복 횟수)에 LLM이 앵커링되고 건수형 상한(§29)도 의도적으로 안 달아
건수형이 부푼다. 따라서 req-actions보다 **크게 나오는 것이 정상**이며,
그 간격이 곧 "직행 자와 궤적 자의 거리"다 (10세션 실측 +37min/26%,
전부 닻 없는 건수형에서 발생 — draft/edit/read는 세 방식 동일).
