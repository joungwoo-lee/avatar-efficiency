...
### 쉬운 요약 — 두 자(尺)가 왜 다르고, 뭘 고쳤고, 뭘 못 고쳤나

사람 시간을 재는 자가 2개다. 자A(요구사항 기반)는 "이 결과물을 사람이 만들려면
무슨 일을 얼마나 해야 하나"로 계산하고, 자B(행동 기반, primitive)는 "사람이
하면 무슨 행동을 몇 번 하나 × 행동당 시간"으로 계산한다. 같은 세션인데 자B가
자A의 1/3이 나오기도, 3배가 나오기도 했다.

고친 것 2개:
1. 읽고 판단하는 일(채점)이 자B에서 너무 작게 나왔다 → 자B에게 "읽어야 할
   자료가 몇 단어인지"를 안 알려준 게 원인. 알려주게 고침 → 절반쯤 회복.
2. 같은 파일을 여러 번 고쳐 쓰면 고칠 때마다 분량을 전부 더해서 실제보다
   크게 셌다 → 마지막 완성본 기준으로 세게 고침 + "쓴 분량을 수정 분량으로
   또 세지 마라" 규칙 추가.

못 고친 것 2개 (구조적 한계 — 이 유형은 자A만 쓴다):
1. 문서 쓰는 일: AI는 말이 많다. AI가 1.5만 단어를 썼다고 사람도 그만큼 쓸
   필요는 없는데, 자B는 "쓰인 단어수 × 단가"라서 AI의 수다를 사람 노동으로
   계산한다. 얼마나 깎아야 할지 근거가 없어 부합 불가.
2. 조사하는 일: 조사는 파일을 안 남긴다. 자B는 남은 파일을 보고 세는데,
   찾아 읽고 확인한 수고는 흔적이 없어 1/3로 작게 나온다.

결론: 코드 짜는 일은 두 자가 비슷하게 나와 병행 가능. 읽기·조사·문서 일은
자A를 믿는다. 완전히 맞추려면 진짜 사람이 그 일을 해본 시간 기록이 필요하다.
