...
| 파일 | 역할 |
|---|---|
| `core.py` | 핵심 함수 2개: `build_org_function_db()`, `get_org_functions()` (+ `get_org_functions_by_name()`) |
| `api.py` | FastAPI REST API (`/build`, `/functions`) |
...
1. **빌드** — `build_org_function_db()`
   ...
   - 원본 시트의 1행 라벨(=DB 열 이름)로 팀/그룹/파트/Function1/Function2 열을 지정받아 그 5열만 SELECT.
...
```python
from core import build_org_function_db, get_org_functions, get_org_functions_by_name

# 1) 조직기능 DB 구축 (열 라벨 = 원본 시트 1행의 값)
...
| `SSOT_COL_TEAM/GROUP/PART/FUNC1/FUNC2` | | 열 라벨 기본값 (build 인자가 우선) |
