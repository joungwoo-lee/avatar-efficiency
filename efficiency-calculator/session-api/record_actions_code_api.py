...
방법론 (§32):
    ...
    LLM의 잔여 역할(행동 종류 선택)을 고정 대응 규칙으로 치환:
      ...
      think   = 전략 생각 (§53) — 지시 직후 첫 응답의 생각 토큰 × 요율
                (0.00417 = §57 정독 동속 0.005의 1.2배속, §86), 서브에이전트
                기록 포함 (§68). (구 포맷은 토큰 수가 없어 건당 1.25분 고정,
                §58의 1.5분을 §86에서 1.2배속)
...
THINK_FALLBACK_MIN = 1.25     # 구 포맷(토큰 미기록) 전략 생각 지점의 건당 시간 (§58 1.5 → §86 1.2배속 1.25)
...
_THINK_DEFAULT_SPEC = {"unit": "word_count", "min_per_unit": 0.00417}
...
def collect_strategy_thinking(jsonl_path, subagent_paths=(), count_window=None):
    ...
    선별 규칙 (§53):
      ...
      건당 THINK_FALLBACK_MIN(1.25분, §86) 고정으로 계상한다(§58) — 실측 평균
