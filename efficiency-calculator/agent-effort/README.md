...
2. **AI 몫은 실제 소모 시간 기준.** 병렬 실행(서브에이전트)은 메인 타임라인에
   ...
   필요하면 옵션으로만 켠다.
3. **파일 확인은 결과물 단위·로그·상한** (hitl_review_model.file_check, §77):
   확인 시점(실질 지시 턴·세션 끝)마다 그 구간에 파일 생성·수정이 있으면
   유형 무관 1건 = `min(2.0, 0.5 × ln(1 + 구간에 쓴 단어/100))`. 사람은
   수단(생성 스크립트)이 아니라 결과물을 보고, 실측(§63)은 확인 비용이
   규모와 거의 무관했다. 채팅 보고는 **결론(턴 마무리 답변)만 정독**(건별
   200단어 상한)하고 진행 보고는 훑기. 구 코드 동작 확인 2.0·규모 비례
   대조·문서 정독·파일당 표본 확인은 폐지. **검증 위임 (§78)**: 확인 시점의
   마지막 테스트가 통과면 그 이전에 쓴 코드 파일은 확인 대상에서 제외
   (`automation_saved_min` = 줄어든 확인비). `file_check` 키가 없으면 구식
   경로로 폴백.
   correct(interrupt × 4.0)도 §77에서 폐지 — instruct와 이중 계상.
