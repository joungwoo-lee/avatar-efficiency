...
**1차 해법은 입력 케이스 분리다** (방법론 문서 §2.1): 오해석의 근원이 "트랜스크립트용
복원 로직을 아바타 업무 정의에 유용한 것"이었으므로, 첫 단계를 아바타 특화
변환기(Prompt A-avatar)로 교체했다. 아바타 입력의 확정 의미(스킬=기존 도구, 반복
업무 1회분, 명시 산출물만)가 프롬프트 규칙이 아니라 **입력 스키마의 전제**가 되면서
"자동화→구축" 오해석이 원천에서 사라진다 — 추가 호출 비용 없음(2회 유지). 라이브
검증: 메일·RCA×2·보고서 4케이스 SW 오분류 0 (critic·케이스별 few-shot 없이).

2차 안전망으로 방법론 문서 §7.1의 Pass D(Consistency Critic)를 구현했다
(`critic=True` 옵션, 기본 OFF — 호출 1회 추가 비용):
