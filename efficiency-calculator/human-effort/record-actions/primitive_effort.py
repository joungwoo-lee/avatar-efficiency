...
- "[입력 자료 규모]"가 있으면 read count의 근거로 사용하라 — 지시문에 포함된
  자료는 사람도 읽어야 한다.
- "[조사 자료 규모]"는 조사·검증 노동이 있었다는 신호다 — read/search 행동을
  목록에 포함하라. 단 read의 규모 숫자는 대략값이면 된다: 최종 수치는
  시스템이 기록 실측으로 확정한다 (결과에 기여한 자료는 실측 그대로 정독,
  탐색 중 훑은 것은 대폭 할인, 헛읽기는 0). 비율을 임의로 정하지 마라.
...
- **사람의 경로를 새로 구성하라.** 업무 설명에 AI가 실제 수행한 기록(도구 호출
  ...
  사람은 필요한 확인 몇 번만 한다.
- 기록 속 사용자와 AI의 대화 왕복을 communicate로 계상하지 마라 — 사람 혼자
  하는 시나리오에는 그 대화가 없다. communicate는 할일 자체가 소통 산출물
  (메일·회신·보고 발송 등)일 때만 잡는다.
