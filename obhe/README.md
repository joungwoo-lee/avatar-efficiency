...
## 리뷰·답변 세션 — 파일 산출물이 0건일 때 (§5.4)

코드 리뷰·분석·질의응답 세션은 파일을 안 만든다. 이때 산출물은 **대화 속 최종 답변**이다.

- job에 Write/Edit 기록이 **0건이면 자동 발동**: 최종 assistant 답변을
  `(대화 답변)` artifact로 추출 (transcript 원본 → HIGH_CONFIDENCE)
- 읽기·검색 workload는 **실측**: Read/Grep/Glob 기록에서 읽은 파일 수·LOC·검색
  횟수를 코드가 세서 `review_evidence`로 첨부 — LLM은 이 수치를 그대로 쓴다
- **편집이 있는 세션엔 아무 변화 없음** — 코딩 세션의 최종 답변은 작업 보고라
  diff와 이중계산되므로 기본 제외
- 혼합 세션(리뷰+일부 수정)은 기본 파일 산출물만(과소평가 방향 안전).
  `--include-answers`로 opt-in하면 답변도 추가하되, "diff에 대응물이 없는
  독립 결과(발견·판정·권고)만 세라" 규칙으로 이중계산 차단

라이브 실측: 리뷰 전용 세션 → 결함 지적 3건으로 분해, 읽기량 실측치 인용, RHE P50 1.07h.
