...
| 순서 | 하는 일 | 쉽게 말하면 |
|---|---|---|
| 1 | 진짜 쓰인 것만 남긴다 | AI가 만들다 버린 것, 중간 산출물, 안 읽을 분량은 계산에서 뺀다 (§3) |
| 2 | 두 가지 시간을 구분한다 | "저걸 통째로 따라 만드는 시간"과 "같은 목적을 사람답게 달성하는 시간"은 다르다. 계산에는 후자를 쓴다 (§4) |
| 3 | 결과물을 알맹이 단위로 센다 | 페이지 수가 아니라 "확인된 사실 31개, 결론 6개, 차트 5개"처럼 센다 (§5) |
| 4 | 사람이 밟았을 길을 그린다 | "자료 찾고 → 읽고 → 비교하고 → 쓰고 → 검토한다" — AI가 실제로 한 방식이 아니라 사람의 정상적인 일 순서다 (§6) |
| 5 | 각 일의 양을 센다 | "자료 15건, 확인할 주장 31개"처럼 결과물에서 셀 수 있는 숫자로 남긴다. 나중에 근거를 따질 수 있다 (§7) |
| 6 | 양 × 단가로 시간을 낸다 | "주장 1개 확인 = 3분"처럼 일마다 단가표가 있고, 어려우면 추가 시간이 붙는다. 단가표는 외부 파일(rate_card.json)이라 조직 데이터로 바꿀 수 있다 (§8·§10) |
| 7 | 사람다운 비용도 넣는다 | 사람도 검토하고, 사람도 고쳐 쓴다. 그 시간을 따로 더한다 (§13·§14) |
| 8 | 세 번 물어서 흔들림을 잡는다 | 4번의 "길 그리기"를 3번 반복해 중간값을 쓴다. 답이 너무 다르면 사람 검토로 넘긴다 (§17) |
| 9 | 답은 범위로 준다 | "정확히 17.3시간"이 아니라 "보통 15시간, 넉넉히 21시간, 믿을 만한 정도 B" (§18·§19) |
