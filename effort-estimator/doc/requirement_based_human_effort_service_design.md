...
#### 입력 변형: 사전 작업 지침서 (v0.6)

업무 **실행 전** 산정에는 트랜스크립트 대신 작업 지침서(할일+역할+업무상세+스킬 자유
텍스트)를 입력으로 받을 수 있다. 이 경우 수행상태(delivered/partial)는 존재하지 않으므로
모든 Requirement의 status는 `planned`이며, 지침서에 명시된 요청 범위 전체를 산정한다.
나머지 파이프라인(작업분해→매핑→결정론적 계산)은 동일하다.
